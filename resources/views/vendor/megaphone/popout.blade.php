<div x-cloak x-show="open" @click.outside="open = false" class="w-full fixed z-50 top-0 right-0 h-full overflow-x-hidden transform translate-x-0 transition ease-in-out duration-700" id="notification">
    ...
    <div class="2xl:w-4/12 bg-gray-50 shadow-md h-screen overflow-y-auto p-8 pt-3 absolute right-0 z-30">
        <div class="flex items-center justify-content-end">
            <p tabindex="0" class="focus:outline-none text-2xl font-semibold leading-6 text-gray-800 header-notification">Notifications</p>
            <button role="button" aria-label="close modal" class="focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 rounded-md cursor-pointer button-close-notification" @click="open = false">
        ...
        @if ($unread->count() > 0)
            <h2 tabindex="0" class="focus:outline-none text-sm leading-normal border-b border-gray-300 text-gray-600 header-notification-unread">
            ...
            @foreach ($unread as $announcement)
                <div class="w-full p-3 mt-4 bg-white rounded flex flex-shrink-0 {{ $announcement->read_at === null ? "drop-shadow shadow border" : ""  }}">
                    ...
                    @if($announcement->read_at === null)
                        <button role="button" aria-label="Mark as Read" class="w-8 h-6 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 rounded-md cursor-pointer"
